...
{
mv = cw.visitMethod(ACC_PUBLIC, "apply", "(Lmodels/rec;)Lmodels/MyRecord;", null, null);
mv.visitCode();
mv.visitTypeInsn(NEW, "models/MyRecord");
mv.visitInsn(DUP);
mv.visitVarInsn(ALOAD, 1);
...
mv.visitMethodInsn(INVOKEVIRTUAL, "models/MyRecord$", "apply", "(Lmodels/rec;)Lmodels/MyRecord;");
mv.visitInsn(ARETURN);
mv.visitMaxs(2, 2);
mv.visitEnd();
}
